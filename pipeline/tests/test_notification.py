...
def test_notify_calls_osascript_on_macos():
    with patch("autosplat.notification.platform.system", return_value="Darwin"):
        with patch("autosplat.notification.subprocess.run") as mock_run:
...
def test_notify_no_op_on_non_macos():
    with patch("autosplat.notification.platform.system", return_value="Linux"):
        with patch("autosplat.notification.subprocess.run") as mock_run:
...
def test_notify_graceful_on_subprocess_exception():
    with patch("autosplat.notification.platform.system", return_value="Darwin"):
        with patch("autosplat.notification.subprocess.run", side_effect=FileNotFoundError("osascript not found")):
            # Should not raise
